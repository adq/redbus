...
public class BookmarksActivity extends ListActivity implements IStopDbUpdateResponseListener, OnCancelListener, ICommonResultReceiver, ITrafficNewsResponseListener
{
    ...
    public void onAsyncCheckUpdateSuccess(int requestId, long updateDate, String updateUrl) {
        if (requestId != stopDbExpectedRequestId)
            return;
        ...
        if (!isManualUpdateCheck) {
            ...
            stopDbExpectedRequestId = StopDbUpdateHelper.getUpdate(updateDate, updateUrl, BookmarksActivity.this);
        } else {
            final long updateDateF = updateDate;
            final String updateUrlF = updateUrl;
            new AlertDialog.Builder(this)
                ...
                .setPositiveButton(android.R.string.ok,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int whichButton) {
                                if (busyDialog != null)
                                    busyDialog.show(BookmarksActivity.this, "Downloading bus data update...");
                                stopDbExpectedRequestId = StopDbUpdateHelper.getUpdate(updateDateF, updateUrlF, BookmarksActivity.this);
                            }
                        })
                .setNegativeButton(android.R.string.cancel, null)
                .show();
